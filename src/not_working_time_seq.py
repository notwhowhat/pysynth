...
while True:
    ...
    # if note is on now?
    # continue playing it
    # if next note trigger now?
    # stop note if on
    # trigger note
    # else you should continue

    if bar_start_time + bar_time < time.time_ns():
        bar_start_time = time.time_ns()
        print(bar_start_time)

    # doesnt work because it is checked to unfrequently: to slow!
    for i, note in enumerate(time_sequence_starts):
        if int(note + bar_start_time  // 1000)== time.time_ns() // 1000:
            note_trigger = True
            frequency = 440
            velocity = 1
            note_start = True
            ...
            if time_sequence_durations[i] > 0:
                if bar_start_time + note + time_sequence_durations[i] > time.time_ns():
                    note_trigger == True
                #else:
                #    note_trigger == False
        #else:
        ...
        '''
        if time_sequence_durations[i] > 0:
            if note + bar_start_time + time_sequence_durations[i] < time.time_ns():
                note_trigger == False
            else:
                note_trigger == True
        else:
