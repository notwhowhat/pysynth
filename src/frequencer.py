...
sequence_pattern = [True, False, True, False]

sequence_frequencies = [440, 0, 220, 0]
step_sequence = 0

if len(sequence_pattern) == len(sequence_frequencies):
...
while True:
    ...
    if step_sequence >= len(sequence_pattern):
        step_sequence = 0

    if sequence_pattern[step_sequence] == True:
        note_trigger = True
    ...
    samples = 0.1 * scitooth(2.0 * np.pi / (SAMPLE_RATE / frequency) * empty_data)
